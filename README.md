...
클라이언트 요청을 받아서 처리하고, 데이터베이스와 상호작용하여 응답을 생성하는 이 애플리케이션의 핵심 원리입니다.

각 선수들을 검색하고, 실시간으로 스텟을 업데이트 하기위해서 수정중입니다.

아직 구성이나 기능적인 부분에서 많은 아쉬움과 결함이 있지만 First Project인 만큼 배울점이 정말 많은 Project 였습니다.

웹크롤링, reDicect, request, response, 등 백앤드의 기초적인 지식과 MongoDB를 연동하여 DB의 기본적인 작동방식 연동방식을 알게되었습니다.
...
1. 복제: `git clone https://github.com/당신의_유저네임/당신의-레포.git`
2. npm install
3. MongoDB 데이터베이스 설정 및 프로젝트에서 연결 문자열 업데이트
4. 서버 시작: `npm start`
